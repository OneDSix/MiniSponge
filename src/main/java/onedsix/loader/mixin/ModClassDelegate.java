...
import onedsix.loader.core.GameTransformer;
...
final public class ModClassDelegate {
	...
	public void initializeTransformers() {
		...
		if (mixinTransformer == null) {
			try { // reflective instantiation for older mixins versions
				...
				log.error("Can't create Mixin transformer through reflection (only applicable for 0.8-0.8.2): ", e);

				// both lookups failed (not received through IMixinService.offer and not found through reflection)
				throw new IllegalStateException("mixins transformer unavailable?");
			}
		}
	...
	public void addCodeSource(Path path) {
		...
		log.info(String.format("added code source %s", path));
	}
		...
		synchronized (classLoader.getClassLoadingLockFwd(name)) {
			...
			if (c == null) {
				...
				if (c == null) {
					...
					log.info(String.format("loaded class %s into target", name));
				}
			}
				...
				if (name.startsWith("java.")) { // fast path for java.** (can only be loaded by the platform CL anyway)
					...
					if (c == null) { // not available locally, try system class loader
						...
						if (url == null) { // no .class file
							try {
								c = PLATFORM_CLASS_LOADER.loadClass(name);
								log.info(String.format("loaded resources-less class %s from platform class loader", c));
							} catch (ClassNotFoundException e) {
								log.error(String.format("can't find class %s", name), e);
								throw e;
							}
						...
						if (Mods.logClassLoad) log.info(String.format("loaded class %s", name));
					}
				}
			}
	...
	/**
	 * Runs all the class transformers except mixins.
	 */
	...
	private byte[] getRawClassByteArray(String name, boolean allowFromParent) throws IOException {
		...
		if (url == null) {
			...
			if (!isValidParentUrl(url, name)) {
				log.info(String.format("refusing to load class %s at %s from parent class loader", name, getCodeSource(url, name)));
